    ...
    <>
      ...
      <nav className="hidden md:flex bg-light-mint shadow-sm">
        <div className="container mx-auto px-4 py-2 justify-center flex space-x-6 font-medium">
          <Link
            to="/"
            className={`hover:text-muted-green transition ${location.pathname === "/" ? "font-bold text-muted-green" : "text-deep-green"
              }`}
          >
            Home
          </Link>
          {categories.map((cat) => (
            <Link
              ...
              className={`hover:text-muted-green transition ${location.pathname === `/category/${cat.slug}` ? "font-bold text-muted-green" : "text-deep-green"
                }`}
            >
              {cat.name}
            </Link>
      ...
      {mobileOpen && (
        <ul className="md:hidden flex flex-col space-y-2 px-4 pb-4 font-medium bg-light-mint">
          <li>
            <Link
              to="/"
              className={`block px-2 py-1 hover:text-muted-green transition ${location.pathname === "/" ? "font-bold text-muted-green" : "text-deep-green"
                }`}
          ...
          {categories.map((cat) => (
            <li key={cat._id}>
              <Link
                to={`/category/${cat.slug}`}
                className={`block px-2 py-1 hover:text-muted-green transition ${location.pathname === `/category/${cat.slug}` ? "font-bold text-muted-green" : "text-deep-green"
                  }`}
                onClick={() => setMobileOpen(false)}
              >
                {cat.name}
